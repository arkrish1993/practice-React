...
const Cart = (props) => {
  ...
  const submitOrderHandler = (userData) => {
    ...
    fetch("https://udemy-http-1c237-default-rtdb.firebaseio.com/orders.json", {
      method: "POST",
      body: JSON.stringify({
        user: userData,
  ...
  const orderSubmitted = <p>Order saved successfully!</p>;

  return (
    <Modal onClose={props.onClose}>
      {!isSubmitting && !didSubmit && cardModalContent}
      {isSubmitting && !didSubmit && submittingOrderData}
